...
export function Hero() {
  // Tall outer section gives ScrollTrigger room to scrub through all frames.
  // Inner is sticky to top of viewport so canvas + hero copy stay pinned
  // while the user scrolls through the 200vh scroll runway.
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        {/* Frame-scrub bloom background */}
        <HeroFrames triggerRef={sectionRef} />

        {/* Soft top vignette so the type sits crisply against the night sky */}
        <div
          aria-hidden
          className="absolute inset-x-0 top-0 h-[50%] bg-gradient-to-b from-background via-background/40 to-transparent"
        />

        {/* Hero content overlays the canvas */}
        <div className="absolute inset-0 z-10 flex h-screen flex-col items-center justify-between px-6 md:px-12 pt-28 md:pt-32 pb-10 md:pb-12 text-center">
          ...
          {/* Spec strip — sits at the bottom of the viewport, over the water reflection */}
          <motion.dl
            {...fade(0.45)}
